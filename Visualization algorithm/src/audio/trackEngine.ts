...
class TrackEngine {
  ...
  // ============ 麦克风 / 录音 ============
  currentRecordingDeviceId: string | null = null;
  currentRecordingLabel: string = 'Microphone';
    ...
    try {
      ...
      if (deviceId) {
        ...
        if (track) {
          ...
          this.currentRecordingLabel = track.label || 'System Audio';
        }
      } else {
        const track = this.micStream.getAudioTracks()[0];
        this.currentRecordingDeviceId = deviceId || 'default';
        this.currentRecordingLabel = track?.label || 'Microphone';
      }
      ...
      console.error('audio input denied', e);
  ...
  stopMic() {
    ...
    this.currentRecordingLabel = 'Microphone';
  }

  async startRecording(deviceId?: string): Promise<void> {
    const ok = await this.requestMic(deviceId);
