...
export interface MVSpectrumLayoutConfig {
  barCount: number;
  radius: number;
  barWidth: number;
  mirror: boolean;
...
interface MVState {
  ...
  previewAspect: 'auto' | 'landscape' | 'portrait';
  rackLocked: boolean;
  collapsedSlotIds: number[];
  ...
  moveSlotUp: (slotId: number) => void;
  moveSlotDown: (slotId: number) => void;
  renameSlot: (slotId: number, label: string) => void;
  toggleRackLock: () => void;
  toggleSlotCollapsed: (slotId: number) => void;
  ...
  setPreviewAspect: (aspect: 'auto' | 'landscape' | 'portrait') => void;
...
function getDefaultSlotParams(
  type: EffectType,
  global: MVGlobalParams,
  spectrum: MVSpectrumLayoutConfig,
): EffectParams {
  switch (type) {
    ...
    case 'spectrum-bar':
      return {
        ...
        fitWidth: 1,
        fillHeight: 0.92,
        offsetX: spectrum.offsetX,
        offsetY: spectrum.offsetY,
...
function syncSlotsFromState(
  existingSlots: EffectSlot[],
  state: {
  ...
  return existingSlots.map((slot) => {
    if (slot.id === 0 && slot.type === 'background-gradient') {
      return { ...slot, params: { ...slot.params, backgroundColor: state.global.backgroundColor, primaryColor: state.global.primaryColor, secondaryColor: state.global.secondaryColor, glowColor: state.global.glowColor, hue: state.global.hue, saturation: state.global.saturation, brightness: state.global.brightness } };
    }
    if (slot.id === 1 && (slot.type === 'spectrum-bar' || slot.type === 'spectrum-circular')) {
      return { ...slot, params: slot.type === 'spectrum-bar'
        ? { ...slot.params, count: state.spectrumSettings.barCount, width: state.spectrumSettings.barWidth, direction: mirror ? 2 : 0, offsetX: state.spectrumSettings.offsetX, offsetY: state.spectrumSettings.offsetY, scale: state.spectrumSettings.scale, sensitivity: state.global.sensitivity, smoothing: state.global.smoothing, glow: state.global.glow, colorA: state.spectrumSettings.colorA, colorB: state.spectrumSettings.colorB, glowColor: state.spectrumSettings.glowColor }
        : { ...slot.params, count: state.spectrumSettings.barCount, radius: state.spectrumSettings.radius, offsetX: state.spectrumSettings.offsetX, offsetY: state.spectrumSettings.offsetY, scale: state.spectrumSettings.scale, sensitivity: state.global.sensitivity, smoothing: state.global.smoothing, glow: state.global.glow, colorA: state.spectrumSettings.colorA, colorB: state.spectrumSettings.colorB, glowColor: state.spectrumSettings.glowColor } };
    }
    if (slot.id === 2 && slot.type === 'particle-burst') {
      return { ...slot, params: { ...slot.params, density: 26 + state.global.sensitivity * 18, speed: 40 + state.global.glow * 30, size: 1.8 + state.global.glow * 2.2, colorA: state.global.primaryColor, colorB: state.global.secondaryColor, glowColor: state.global.glowColor } };
    }
    if (slot.id === 3 && slot.type === 'text-overlay') {
      return { ...slot, params: { ...slot.params, showTitle: state.text.showTitle, songTitle: state.text.songTitle, artistName: state.text.artistName, textPosition: state.text.position, fontSize: state.text.fontSize, textGlow: state.text.textGlow, offsetX: state.text.offsetX ?? 0, offsetY: state.text.offsetY ?? 0, textColor: state.global.textColor, glowColor: state.global.glowColor } };
    }
    return slot;
  });
}
...
function createSlotsFromState(state: {
  ...
  const slots: EffectSlot[] = [
    ...
    },
    {
      ...
      params: spectrumType === 'spectrum-bar'
        ? {
            ...
            fitWidth: 1,
            fillHeight: 0.92,
            offsetX: state.spectrumSettings.offsetX,
            offsetY: state.spectrumSettings.offsetY,
            scale: state.spectrumSettings.scale,
    ...
    },
    {
      ...
      params: {
        ...
        offsetX: state.text.offsetX ?? 0,
        offsetY: state.text.offsetY ?? 0,
  ...
  for (let slotId = 4; slotId < 8; slotId += 1) {
    slots.push({
      id: slotId,
      type: 'none',
      enabled: false,
  ...
  if (!state.slotOrder || state.slotOrder.length === 0) {
    return slots;
  }

  const orderMap = new Map(state.slotOrder.map((slotId, index) => [slotId, index]));
  return slots.slice().sort((a, b) => (orderMap.get(a.id) ?? a.id) - (orderMap.get(b.id) ?? b.id));
}
...
function createInitialState() {
  ...
  const spectrumSettings = {
    ...
    offsetY: 0,
    scale: 1,
    colorA: getMVTheme(template.theme).primaryColor,
    colorB: getMVTheme(template.theme).secondaryColor,
    glowColor: getMVTheme(template.theme).glowColor,
  };
  const base = {
  ...
  return {
    ...
    effects: {
      ...
      spectrum: {
        ...
        offsetY: state.spectrumSettings.offsetY,
        scale: state.spectrumSettings.scale,
        colorA: state.spectrumSettings.colorA,
        colorB: state.spectrumSettings.colorB,
        glowColor: state.spectrumSettings.glowColor,
...
export const useMVStore = create<MVState>((set, get) => {
  ...
  return {
    ...
    toggleRackLock: () => set(state => ({ rackLocked: !state.rackLocked })),
    renameSlot: (slotId, label) => set(state => {
      const slots = state.slots.slice();
      const index = slots.findIndex(slot => slot.id === slotId);
      if (index < 0) return state;
      slots[index] = { ...slots[index], label: label || undefined };
      return { slots };
    }),
    toggleSlotCollapsed: (slotId) => set(state => ({
      collapsedSlotIds: state.collapsedSlotIds.includes(slotId)
        ? state.collapsedSlotIds.filter(id => id != slotId)
        : [...state.collapsedSlotIds, slotId],
    })),

    setSlotType: (slotId, type) => {
      if (!SUPPORTED_EFFECTS.includes(type)) return;
      const state = get();
      if (state.rackLocked) return;
      const defaultParams = getDefaultSlotParams(type, state.global, state.spectrumSettings);
      set(state2 => ({
        slots: state2.slots.map(slot =>
          slot.id === slotId
            ? { ...slot, type, enabled: type === 'none' ? false : slot.enabled || true, params: defaultParams }
            : slot
        ),
      }));
    },
    ...
    toggleSlot: (slotId) => {
      set(state => {
        if (state.rackLocked) return state;
        return {
    ...
    moveSlotDown: (slotId) => {
      set(state => {
        if (state.rackLocked) return state;
        const index = state.slots.findIndex(slot => slot.id === slotId);
        if (index < 0 || index >= state.slots.length - 1) return state;
        const slots = state.slots.slice();
        const [slot] = slots.splice(index, 1);
        slots.splice(index + 1, 0, slot);
        return { slots };
    ...
    updateGlobal: (params) => {
      set(state => {
        const nextGlobal = {
          ...
          opacity: clamp(params.opacity ?? state.global.opacity, 0.1, 1),
        };
        const nextState = {
          ...
          slotOrder: state.slots.map(slot => slot.id),
          currentTheme: state.currentTheme,
          global: nextGlobal,
        };
        return {
          global: nextGlobal,
          slots: syncSlotsFromState(state.slots, nextState),
        };
      });
    },
    ...
    applyTemplate: (templateId) => {
      ...
      const runtimeSeed = {
        ...
        slotOrder: get().slots.map(slot => slot.id),
      };
    ...
    applyTheme: (themeId) => {
      set(state => {
        ...
        return {
          ...
          currentTheme: theme,
          global: nextGlobal,
          slots: syncSlotsFromState(state.slots, {
            dynamicBackground: state.dynamicBackground,
            text: state.text,
            global: nextGlobal,
            spectrumSettings: state.spectrumSettings,
    ...
    updateText: (params) => {
      set(state => {
        const nextText = {
          ...state.text,
          ...params,
          showTitle: state.rackLocked && params.showTitle !== undefined ? state.text.showTitle : (params.showTitle ?? state.text.showTitle),
          position: isTextPosition(String(params.position ?? state.text.position))
            ? (params.position ?? state.text.position)
            : state.text.position,
          offsetX: clamp(params.offsetX ?? state.text.offsetX ?? 0, -0.40, 0.40),
          offsetY: clamp(params.offsetY ?? state.text.offsetY ?? 0, -0.40, 0.40),
        };
        return {
          text: nextText,
          slots: syncSlotsFromState(state.slots, {
    ...
    updateDynamicBackground: (params) => {
      set(state => {
        const nextDynamicBackground = {
          ...
          ...params,
          enabled: state.rackLocked && params.enabled !== undefined ? state.dynamicBackground.enabled : (params.enabled ?? state.dynamicBackground.enabled),
        };
        return {
    ...
    updateSpectrumSettings: (params) => {
      set(state => {
        const nextSpectrumSettings = {
          ...state.spectrumSettings,
          ...params,
          offsetX: clamp(params.offsetX ?? state.spectrumSettings.offsetX, -0.40, 0.40),
          offsetY: clamp(params.offsetY ?? state.spectrumSettings.offsetY, -0.40, 0.40),
          scale: clamp(params.scale ?? state.spectrumSettings.scale, 0.15, 3.0),
        };
        return {
          spectrumSettings: nextSpectrumSettings,
          slots: syncSlotsFromState(state.slots, {
    ...
    importProject: (project) => {
      ...
      const text: MVTextConfig = {
        ...
        offsetX: clamp(project.text.offsetX ?? 0, -0.40, 0.40),
        offsetY: clamp(project.text.offsetY ?? 0, -0.40, 0.40),
      };
      ...
      const spectrumSettings = {
        ...
        offsetX: clamp(project.effects.spectrum.offsetX ?? 0, -0.40, 0.40),
        offsetY: clamp(project.effects.spectrum.offsetY ?? 0, -0.40, 0.40),
        scale: clamp(project.effects.spectrum.scale ?? 1, 0.15, 3.0),
        colorA: project.effects.spectrum.colorA ?? getMVTheme(themeId).primaryColor,
        colorB: project.effects.spectrum.colorB ?? getMVTheme(themeId).secondaryColor,
        glowColor: project.effects.spectrum.glowColor ?? getMVTheme(themeId).glowColor,
      };
      const runtimeSeed = {
      ...
      const global = {
        ...
        opacity: clamp(project.effects.spectrum.opacity ?? 1, 0.1, 1),
      };
      set({
        currentTemplateId: templateId,
        currentThemeId: themeId,
