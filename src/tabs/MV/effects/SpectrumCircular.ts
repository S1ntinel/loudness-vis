...
export class SpectrumCircularEffect extends MVEffect {
  ...
  render(context: RenderContext, audioData: AudioData): void {
    ...
    const maxBarLength = Math.max(8, Math.min(width, height) * 0.35 * scale - radius * scale * 0.35);
    ...
    for (let i = 0; i < count; i++) {
      ...
      // 绘制条形
      const x1 = centerX + Math.cos(angle) * scaledInnerRadius;
      const y1 = centerY + Math.sin(angle) * scaledInnerRadius;
      const x2 = centerX + Math.cos(angle) * (scaledInnerRadius + barLength);
      const y2 = centerY + Math.sin(angle) * (scaledInnerRadius + barLength);
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(centerX, centerY, scaledInnerRadius, 0, Math.PI * 2);
    ctx.stroke();
  }
}
