...
export class LyricsKaraokeEffect extends MVEffect {
  ...
  readonly description = '带字级进度高亮和翻译显示的卡拉OK歌词层';
  ...
  private getMergedLines(): LyricLine[] | null {
    ...
    const trId = assets.lyricsTranslationVisible ? assets.activeLyricsTranslationId : null;
  ...
  render(context: RenderContext, _audioData: AudioData): void {
    ...
    const activeColor = this.getStringParam('activeColor', '#60F2FF'); // VFD Cyan
    const inactiveColor = this.getStringParam('inactiveColor', 'rgba(96, 242, 255, 0.3)');
    const glowColor = this.getStringParam('glowColor', '#60F2FF');
    const glow = this.getNumberParam('glow', 0.8);
    let fontFamily = this.getStringParam('fontFamily', 'monospace');
    ...
    const baseFont = `bold ${fontSize}px ${fontFamily}, "MiSans", sans-serif`;
    const trFont = `500 ${Math.max(14, Math.round(fontSize * translationScale))}px ${fontFamily}, "MiSans", sans-serif`;
    ...
    for (let offset = -1; offset <= 1; offset++) {
      ...
      if (showTranslation && line.translation) {
        ctx.font = trFont;
        ctx.save();
        ctx.shadowBlur = isCurrent ? 8 : 2;
        ctx.globalAlpha = isCurrent ? 0.8 : 0.25;
        ctx.fillStyle = isCurrent ? '#6CFF9A' : inactiveColor;
        ctx.fillText(line.translation, width / 2, y + fontSize * 0.45 + Math.max(14, fontSize * translationScale) * 0.85);
        ctx.restore();
      }
    }
...
function renderWordsLine(
  ...
  for (const w of line.words) {
    ...
    if (nowMs >= w.endMs) {
      ...
      ctx.save();
      ctx.beginPath();
      ctx.rect(x, y - 60, wWidth * progress, 100);
      ctx.clip();
      ctx.fillText(w.text, x, y);
      ctx.restore();
    }
...
function renderSweepingLine(
  ...
  if (progress > 0) {
    ctx.save();
    ctx.beginPath();
    ctx.rect(left, y - 60, width * progress, 100);
    ctx.clip();
    ctx.fillStyle = activeColor;
    ctx.fillText(text, left, y);
    ctx.restore();
