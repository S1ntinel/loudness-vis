...
export default function Stage() {
  ...
  const { slots, global, isPlaying, previewAspect } = useMVStore();
  const hasVisualAsset = useMVAssetsStore(s => (
    (s.activeVideoId !== null && s.videoVisible) ||
    (s.activeImageId !== null && s.imageVisible)
  ));
  ...
  const render = useCallback(() => {
    ...
    ctx.globalAlpha = global.opacity ?? 1;
    
    // 按顺序渲染效果
    const orderedSlots = slots.slice().reverse();

    for (const slot of orderedSlots) {
      const effect = effectsRef.current.get(slot.id);
  ...
  const stageStyle = previewAspect === 'portrait'
    ? { aspectRatio: '9 / 16', height: '100%', width: 'auto', maxWidth: '100%', margin: '0 auto' as const }
    : previewAspect === 'landscape'
      ? { aspectRatio: '16 / 9', width: '100%', height: 'auto', maxHeight: '100%', margin: '0 auto' as const }
      : { width: '100%', height: '100%' };

  return (
    <div className={s.stageContainer} style={stageStyle}>
      <canvas
        ref={canvasRef}
        data-mv-stage="true"
        className={s.canvas}
