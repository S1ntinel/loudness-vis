...
export default function TrackItem({ track }: { track: Track }) {
  ...
  const { playMode, playingIds } = useTrackState();
  const isSelfPlaying = 
    playMode === 'preview' &&
    playingIds.length === 1 &&
  ...
  const previewBtnText = !isSelfPlaying
    ? t('record.previewBtn')
    : trackEngine.playState === 'playing' ? t('record.pauseBtn') : t('record.resumeBtn');
  ...
  useEffect(() => {
    ...
    function paint(w: number, h: number) {
      ...
      const playedColor = cssVar('--vfd-cyan', '#60F2FF');
      const dimColor    = cssVar('--hw-metal', '#A6A49D');
      ...
      for (let px = 0; px < w; px++) {
        ...
        ctx2d.globalAlpha = inside ? 1 : 0.3;
        ctx2d.beginPath();
      ...
      const drawHandle = (x: number) => {
        ...
        ctx2d.stroke();
        ctx2d.fillStyle = cssVar('--vfd-orange', '#F26A2E');
        ctx2d.beginPath();
      ...
      ctx2d.fillStyle = cssVar('--vfd-green', '#6CFF9A');
