          ...
          {activeTab === 'appearance' && (
            <div className={s.section}>
              ...
              <div className={s.row}>
                ...
                <div className={s.btnGroup}>
                  <button
                    className={`${s.btn} ${preset === 'default' ? s.btnActive : ''}`}
                    onClick={() => setPreset('default')}
                    style={{ '--preset-color': '#3b6db5' } as React.CSSProperties}
                  >
                    <span className={s.colorDot} style={{ background: '#3b6db5' }} />
                    {t('settings.default')}
                  </button>
                  <button
                    className={`${s.btn} ${preset === 'green' ? s.btnActive : ''}`}
                    onClick={() => setPreset('green')}
                    style={{ '--preset-color': '#00c8a0' } as React.CSSProperties}
                  >
                    <span className={s.colorDot} style={{ background: '#00c8a0' }} />
                    {t('settings.green')}
                  </button>
                  <button
                    className={`${s.btn} ${preset === 'pink' ? s.btnActive : ''}`}
                    onClick={() => setPreset('pink')}
                    style={{ '--preset-color': '#e8559a' } as React.CSSProperties}
                  >
