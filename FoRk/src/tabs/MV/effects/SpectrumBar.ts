...
export class SpectrumBarEffect extends MVEffect {
  ...
  render(context: RenderContext, audioData: AudioData): void {
    ...
    const startX = fitWidth > 0 ? 0 : (width - totalWidth) / 2;
    const maxBarHeight = height * fillHeight;
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (direction === 0) {
        // 向上
        this.roundRect(ctx, x, height - barHeight, barWidth, barHeight, radius);
      } else if (direction === 1) {
        // 向下
        this.roundRect(ctx, x, 0, barWidth, barHeight, radius);
      } else {
        // 双向
        const halfHeight = barHeight / 2;
        this.roundRect(ctx, x, height / 2 - halfHeight, barWidth, barHeight, radius);
      }
